function subRoiCalculationsTime2Peak(handles)
axes(handles.axes3);
cla(handles.axes3);
image = zeros(512,512);
...
dFnormSmooth=[];
for i=1:size(dFnorm,2);
dFnormSmooth(:,i) = smooth(dFnorm(:,i),5);
end
[~, dFnormSmoothMaxIndex] = max(dFnormSmooth);

clusterIdx = dFnormSmoothMaxIndex;
